...
A sumarização e medidas dos dados podem ser observadas nas abas abaixo:
...
$$\text{id = Identificação do paciente}$$

</center>

```{r echo=FALSE}
cbind(summary(ipec$id)) |> 
  kableExtra::kable()
```

#### ini

$$\text{ini = Data dos diagnósticos da Aids (em dias)}$$
```{r echo=FALSE}
cbind(summary(ipec$ini)) |> 
  kableExtra::kable()
```
...
$$\text{fim = Data do óbito (ou perda do paciente)}$$
```{r echo=FALSE}
cbind(summary(ipec$fim)) |> 
  kableExtra::kable()
```

#### status

$$\text{status = Censura ou óbito}$$
```{r echo=FALSE}
cbind(summary(ipec$status)) |> 
  kableExtra::kable()
```
...
$$\text{sexo = Masculino ou feminino}$$
```{r echo=FALSE}
cbind(summary(ipec$sexo)) |> 
  kableExtra::kable()
```

#### escola

$$\text{escola = Escolaridade}$$
```{r echo=FALSE}
cbind(summary(ipec$escola)) |> 
  kableExtra::kable()
```

#### risco

$$\text{risco = Fatores de risco}$$
```{r echo=FALSE}
cbind(summary(ipec$risco)) |> 
  kableExtra::kable()
```
...
$$\text{acompan = Acompanhamento}$$
```{r echo=FALSE}
cbind(summary(ipec$acompan)) |> 
  kableExtra::kable()
```
...
$$\text{obito = Óbito/Não óbito}$$
```{r echo=FALSE}
cbind(summary(ipec$obito)) |> 
  kableExtra::kable()
```
...
$$\text{anotrat = Ano do início do tratamento}$$
```{r echo=FALSE}
cbind(summary(ipec$anotrat)) |> 
  kableExtra::kable()
```
...
$$\text{tratam = Terapia antirretroviral}$$
```{r echo=FALSE}
cbind(summary(ipec$tratam)) |> 
  kableExtra::kable()
```
...
$$\text{doenca = Doença de apresentação}$$
```{r echo=FALSE}
cbind(summary(ipec$doenca)) |> 
  kableExtra::kable()
```
...
$$\text{propcp = Profilaxia para pneumocistis}$$
```{r echo=FALSE}
cbind(summary(ipec$propcp)) |> 
  kableExtra::kable()
```
...
labs(x = "Horas",y="Probabilidades de Sobreviência") +
